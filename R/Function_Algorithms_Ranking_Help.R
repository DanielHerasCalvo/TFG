...
#'
#' Calculate clustering rankings for the different algoritm's configurations
#' @param Dataset Normalized matrix of continous data
#' @param Algorithm_Configs List of algorithms and configurations
#' @param Metrics_List List of metrics for evaluation
#' @return List with mean ranking, most times winner ranking and dictionary with the algorithms and configurations
#' @examples
#' Dataset <- USArrests
#' Dataset <- na.omit(Dataset)
#' Dataset <- scale(Dataset)
#' Algorithm_Configs <- list(km = list(name = "kmeans", config = list(centers = c(3,4,5), iter.max = 10)), cm = list(name = "Cluster_Medoids", config = list(clusters = c(4,5,6), swap_phase = TRUE)))
#' Metrics_List <- "all"
